...
def bt_search(algo, csp, variableHeuristic, allSolutions, trace, board):
    ...
    if algo == 'BT':
        ...
        solutions = FC(uv, csp, allSolutions, trace, board)
...
def FC(unAssignedVars, csp, allSolutions, trace, board):
    if unAssignedVars.empty():
        soln =  []
        for v in csp.variables():
            soln.append((v, v.getValue()))
    ...
    for val in var.curDomain():
        ...
        if noBreak:
            #updateBoard
            soln = FC(unAssignedVars, csp, allSolutions, trace, board)
            if soln != 'DWO':
                return soln
        board.remove_last_points()
    var.setValue(None)
